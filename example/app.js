...
{		
	//localstorage name and instance
	var ls_name = s1('s1');
	...
	$(document).on('click', '.BTN_Edit_Entry', function(event) 
 	{
		...
		var data = ls_name.getwhere({rec_id:rec_id});
	...
	$('.Screen_Data').on('click', '.BTN_Update_Task', function(event) 
 	{	 
		...
		{		
			...
			var data = ls_name.updatewhere({TaskName:TaskName.val(),TaskDate:TaskDate.val(),TaskDesc:TaskDesc.val()},{rec_id:rec_id});
	...
	$(document).on('click', '.BTN_Delete_Entry', function(event) 
 	{
 		...
 		var ObjArrOptions = 
		{
		  ...
		  {				
			...
			ls_name.removewhere({rec_id:rec_id});
